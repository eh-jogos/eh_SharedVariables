# Basis that can be saved in disk like a custom resource. 
...
var value: Basis = Basis(Vector3.ZERO) setget _set_value, _get_value

# Defautl value in case you're using `is_session_only`
var default_value: Basis = Basis(Vector3.ZERO) setget _set_default_value, _get_default_value
...
func is_class(p_class: String) -> bool:
	return p_class == "BasisVariable" or .is_class(p_class)
...
func _set_value(p_value: Basis) -> void:
...
func _get_value() -> Basis:
	var meta_value = Basis(Vector3.ZERO)
	if _should_reset_value():
		meta_value = get_meta("default_value")
		_set_value(meta_value)
	elif value == Basis(Vector3.ZERO) and has_meta("value"):
...
func _set_default_value(p_value: Basis) -> void:
...
func _get_default_value() -> Basis:
	var meta_value = Basis(Vector3.ZERO)
	if default_value == Basis(Vector3.ZERO) and has_meta("default_value"):
		meta_value = get_meta("default_value")
	else:
