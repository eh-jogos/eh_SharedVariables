...
static func get_dict_from(instance: Object) -> Dictionary:
	...
	if "resource_path" in instance and instance.resource_path != "":
...
static func _handle_loading_object(p_object: Object) -> Object:
	...
	if p_object.is_class("SharedVariable"):
		if p_object.value is Dictionary:
			p_object.value = _handle_loading_dictionary(p_object.value)
		elif p_object.value is Array:
			p_object.value = _handle_loading_array(p_object.value)
