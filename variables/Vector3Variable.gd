# Vector3 that can be saved in disk like a custom resource. 
...
var value: Vector3 = Vector3.ZERO setget _set_value, _get_value

# Defautl value in case you're using `is_session_only`
var default_value: Vector3 = Vector3.ZERO setget _set_default_value, _get_default_value
...
func _set_value(p_value: Vector3) -> void:
...
func _get_value() -> Vector3:
	var meta_value = Vector3.ZERO
	if _should_reset_value():
		meta_value = get_meta("default_value")
		_set_value(meta_value)
	elif value == Vector3.ZERO and has_meta("value"):
...
func _set_default_value(p_value: Vector3) -> void:
...
func _get_default_value() -> Vector3:
	var meta_value = Vector3.ZERO
	if default_value == Vector3.ZERO and has_meta("default_value"):
		meta_value = get_meta("default_value")
	else:
